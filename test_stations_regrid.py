...
def show_result(results, timedblock):
...
def run_main():
    ...
    print
    print 'Method #1 : regrid onto 1x1 cube at each station.'
    with TimedBlock() as tb_1:
        grid_cube = dummy_field((1, 1))
        x_coord = grid_cube.coord('longitude')
        y_coord = grid_cube.coord('latitude')
    ...
    with TimedBlock() as tb_3:
        ...
        grid_cube = dummy_field((N_STATIONS, N_STATIONS))
        grid_cube.coord('longitude').points = sorted_lons
        grid_cube.coord('latitude').points = sorted_lats
        regridded_cube = cube.regrid(grid_cube, scheme)
